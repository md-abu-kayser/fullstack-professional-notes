# 15. useReducer For Complex State
...
After this lesson, you can build predictable, composable user interfaces from small components and explicit state. The focus is **useReducer For Complex State**. A professional implementation makes the happy path clear, handles failure honestly, and remains understandable to the next developer.
...
type Action = { type: "opened" } | { type: "closed" };
const reducer = (state: State, action: Action): State => action.type === "opened" ? { open: true } : { open: false };
...
Build a small **useReducer For Complex State** exercise from your current project. First make the smallest working version, then add one accessibility improvement and one failure-state test. Explain out loud what owns the data, what can change it, and what the user sees while it changes.

## Interview-ready answer

**What should a developer remember about useReducer For Complex State?**  
