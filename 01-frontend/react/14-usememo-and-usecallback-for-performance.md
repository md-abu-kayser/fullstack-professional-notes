# 14. useMemo And useCallback For Performance
...
After this lesson, you can build predictable, composable user interfaces from small components and explicit state. The focus is **useMemo And useCallback For Performance**. A professional implementation makes the happy path clear, handles failure honestly, and remains understandable to the next developer.

## Core ideas

- Measure first with React DevTools Profiler.
- `useMemo`, `useCallback`, and `memo` are optimization tools—not default syntax.
- Keep state local and pass stable, minimal props before adding memoization.
...
Build a small **useMemo And useCallback For Performance** exercise from your current project. First make the smallest working version, then add one accessibility improvement and one failure-state test. Explain out loud what owns the data, what can change it, and what the user sees while it changes.

## Interview-ready answer

**What should a developer remember about useMemo And useCallback For Performance?**  
