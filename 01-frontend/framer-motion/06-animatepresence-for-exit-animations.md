# 06. AnimatePresence For Exit Animations
...
After this lesson, you can add intentional animation that explains state changes without slowing down the interface. The focus is **AnimatePresence For Exit Animations**. A professional implementation makes the happy path clear, handles failure honestly, and remains understandable to the next developer.
...
<AnimatePresence>
  {open && <motion.div key="dialog" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>Saved</motion.div>}
</AnimatePresence>
```
...
Build a small **AnimatePresence For Exit Animations** exercise from your current project. First make the smallest working version, then add one accessibility improvement and one failure-state test. Explain out loud what owns the data, what can change it, and what the user sees while it changes.

## Interview-ready answer

**What should a developer remember about AnimatePresence For Exit Animations?**  
